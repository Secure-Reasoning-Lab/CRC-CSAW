...
- at most one final unified-diff source patch from each patch-generation trial.
...
```math
S_c = w_c \cdot (2AM_cF_c + 6P_c)
```

Here, $w_c$ is the challenge weight: 1 for a delta-scan challenge and 1.25 for a full-scan challenge. $F_c$ is the number of distinct CPVs discovered through valid submitted PoVs. $P_c$ is the number of discovered CPVs for which the corresponding patch-generation trial produces a valid patch.

### Patch-generation trials

After finder-stage PoVs are verified, CRSBench starts a separate patch-generation trial for each discovered CPV. Each trial receives the deduplicated valid PoV or PoV variants that matched its target CPV. A CRS may submit at most one final patch from each patch-generation trial.

A patch contributes one to $P_c$ when it applies, builds, passes the functional tests, and neutralizes all accepted PoV variants for that trial's target CPV. A patch generated for one target CPV does not receive additional qualification credit for incidentally remediating another CPV.

### PoV accuracy multiplier

The accuracy multiplier is calculated only from finder-stage PoVs submitted by the CRS and applies only to finder points. Patches are not included in the accurate or inaccurate submission counts.
...
If $A_c + I_c = 0$, then $AM_c = 1$. Each inaccurate submitted PoV increases $I_c$, so submitting many unverified candidates can reduce the finder score for that challenge. For example, if a team submits one accurate PoV and two inaccurate PoVs, then $A_c = 1$, $I_c = 2$, and $r_c = 1/3$. A fourth PoV that is a reproducible duplicate of the accurate PoV would not change either count. The patch score is unaffected by this multiplier.
